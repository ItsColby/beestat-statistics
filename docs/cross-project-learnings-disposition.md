...
| Sensitive identity changes fail closed | Implemented | The non-reversible account fingerprint detects a different Beestat account. Reconfigure/reauth require confirmation and clear numeric source mappings while preserving timing options. YAML connection replacements now validate first, update only when the fingerprint proves same-account continuity, and otherwise leave saved mappings/history unchanged with an actionable Repair. |
...
| Actionable Repairs and cleanup | Implemented | Missing/wrong-domain enabled override references remain the only mapping Repairs. A scoped entity-registry listener refreshes them immediately when a referenced entity is removed, renamed, or restored; unload removes the listener. A blocked YAML connection replacement raises a separate Repair and clears it after a validated same-account import or YAML removal. Disabled sources remain excluded from mapping checks. |
...
| Maintained HA and repository validation | Implemented | HACS metadata and one exact Linux harness lane require the maintained Core 2026.8.0 baseline. The redundant 2026.7.1 lane and duplicate requirements file are removed; CI still covers Python 3.14, strict mypy, Ruff, unit/HA tests, compile, JSON, privacy, API surface, Hassfest, HACS, actionlint/ShellCheck, zizmor, dependency updates, and a terminal release gate. |
